...
from .options import ValueLabels
from .theme import _THEME


def _hbar(plot, *, highlight, value_strings, xlim, xlabel, ylabel, captions,
          labels: ValueLabels, max_label_chars, ax, **kwargs):
    ...
    if value_strings is not None:
        _draw_value_labels(ax, container, value_strings, padding=labels.padding)
    ...
    ax = _finish(ax, captions, xlabel=xlabel, ylabel=ylabel, grid_axis=grid_axis)
