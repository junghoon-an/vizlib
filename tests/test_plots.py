...
def test_hbar_owned_figure_uses_manual_margins(df):
    ...
    assert not isinstance(ax.figure.get_layout_engine(),
                          __import__("matplotlib.layout_engine",
                                     fromlist=["ConstrainedLayoutEngine"])
...
# --- worst-case cross-preset margin reservation (horizontal bars) -----------
...
def test_margin_reserved_for_largest_regime():
    """Even with the *smaller* default style active, the reserved left region
